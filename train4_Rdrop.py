...
def get_logger(filename=cfg.log + 'train'):
...
def train_fn_r_drop(train_loader, model, optimizer, epoch, scheduler, criterion, device):
    model.train()
    losses = AverageMeter()
    start = end = time.time()
    global_step = 0
    ...
    with tqdm(train_loader, leave=True) as pbar:
        for step, batch in enumerate(pbar):
            ...
            logits_0 = model(input_ids=inp_ids,
                             attention_mask=att_mask,
                             token_type_ids=token_type_ids)

            loss_0 = criterion(logits_0, label.view(-1))

            logits_1 = model(input_ids=inp_ids,
                             attention_mask=att_mask,
                             token_type_ids=token_type_ids)
            loss_1 = criterion(logits_1, label.view(-1))
            ...
            loss.backward()
            grad_norm = torch.nn.utils.clip_grad_norm_(model.parameters(), cfg.MAX_GRAD_NORM or 1e9)
            optimizer.step()
            optimizer.zero_grad()
            global_step += 1
            scheduler.step()
...
def main():
    ...
    train_df['A'] = train_df['A'].map(str)
    train_df['B'] = train_df['B'].map(str)
    train_df['C'] = train_df['C'].map(str)
    train_df['D'] = train_df['D'].map(str)
    train_df['E'] = train_df['E'].map(str)
    ...
    tokenizer.save_pretrained(cfg.r_drop + '/tokenizer/')
    ...
    for fold in cfg.selected_folds:
        ...
        for epoch in range(cfg.epochs):
            ...
            avg_loss = train_fn_r_drop(train_dl, model, optimizer, epoch, scheduler, criterion, device)
