...
from data import DataCollatorForMultipleChoice, prepare_answering_input
...
def main():
    ...
    tokenized_dataset_valid = dataset_valid.map(prepare_answering_input, num_proc=10,
                                                remove_columns=['prompt', 'context', 'A', 'B', 'C', 'D', 'E', 'answer'])
    tokenized_dataset = dataset.map(prepare_answering_input, num_proc=10,
                                    remove_columns=['prompt', 'context', 'A', 'B', 'C', 'D', 'E', 'answer'])
    ...
    tokenized_test_dataset = Dataset.from_pandas(df_valid).map(
        prepare_answering_input, remove_columns=['prompt', 'context', 'A', 'B', 'C', 'D', 'E'])
